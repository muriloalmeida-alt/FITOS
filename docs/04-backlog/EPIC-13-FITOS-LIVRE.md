...
`FIT-103` (#105, sub-issue de #97) implementada: `Student` como auto-referência (`ensureStudentForIndividual`, criado só quando a primeira sessão é iniciada), `startOrResumeIndividualWorkoutSession` (troca a ceremônia de `PlanAssignment`/snapshot da ADR-005 por uma checagem simples de posse + status `ATIVO`) reaproveitando sem alteração `recordSessionResult`/`completeWorkoutSession`/`abandonWorkoutSession`/`getInProgressSessionForStudent`, rotas `/api/minhas-sessoes/*` e páginas próprias (`/painel/meus-treinos/sessao`, botão "Começar treino" em `/painel/meus-treinos/[id]`), banner "Treino em andamento" em `/painel`. Resolve a decisão adiada pela `ADR-006-WORKSPACE-INDIVIDUAL.md` desde o FIT-100. PR mergeado na main (22/09/2026).

`FIT-104` (#107, sub-issue de #97) implementada: histórico/frequência/recordes derivados de `WorkoutSession`/`WorkoutSessionResult` sem novo modelo (`history.ts`); medidas corporais reaproveitando `Assessment`/`BodyMeasurement` (FIT-042) com o próprio praticante como autor; metas em um modelo novo e genérico (`Goal`, por `[studentId, tenantId]`, nunca amarrado a `INDIVIDUAL`); tudo em `/painel/minha-evolucao` (rota própria, nunca `/painel/progresso` do aluno). "Fotos privadas" — também pedida pela História — ficou deliberadamente fora: bloqueada pela decisão pendente de provedor S3 já registrada em `DECISOES-PENDENTES.md`, não uma omissão silenciosa. Detalhe completo em `ADR-008-EVOLUCAO-DO-PRATICANTE-INDIVIDUAL.md`. PR mergeado na main (22/09/2026).

`FIT-105` permanece bloqueada enquanto o EPIC-12 estiver pausado na `FIT-091` — é a única História desta fase dependente do motor de assinatura.

`FIT-106` (#109, sub-issue de #97) implementada: `StudentStatus.VINCULO_ENCERRADO` — terceiro estado, terminal, nunca sinônimo de `INATIVO` (que continua sendo uma pausa reversível) — com `endedAt`/`endReason`/`endedByUserId` aditivos em `Student`, `endStudentBond` (idempotente, nunca sobrescreve o registro original) e `reactivateStudent` agora rejeitando reabrir um vínculo encerrado. `/painel/alunos/[id]/encerrar-vinculo` (confirmação explícita e definitiva) + botão na ficha do aluno. Deliberadamente fora: o controle de acesso granular do ex-aluno (FIT-107/FIT-108) e a comunicação por e-mail (bloqueada pela mesma decisão pendente de "E-mail transacional" já registrada em `DECISOES-PENDENTES.md`) — `requireStudent()` trata o novo estado exatamente como já tratava `INATIVO` por ora. `Student.userId` globalmente único é sinalizado, sem ser resolvido, como bloqueio real para a FIT-109. Detalhe completo em `ADR-009-ENCERRAMENTO-DE-VINCULO.md`. PR mergeado na main (22/09/2026).

`FIT-107` (#111, sub-issue de #97) implementada: verificação, não construção — `endStudentBond` (FIT-106) já só grava três campos no próprio `Student`, nunca toca nenhuma tabela relacionada, e `alunos/[id]/page.tsx` já mostrava o histórico completo ao personal sem condicionar em `status` desde a FIT-014/033/042. `students.preservation.integration.test.ts` prova isso com dados reais de execução, evolução e financeiro (foto antes/depois de `endStudentBond`, igualdade exata). As perguntas sobre o que o personal ainda pode *escrever* para um vínculo encerrado (novas prescrições, avaliações, cobranças) ficam deliberadamente para a `FIT-108`. Detalhe completo na atualização de `ADR-009-ENCERRAMENTO-DE-VINCULO.md`. PR mergeado na main (22/09/2026).

`FIT-108` (#113, sub-issue de #97) implementada: escopo estreito e deliberado — "prescrições" é `assignTrainingPlanToStudent`, e só isso. Agora rejeita (`ESTADO_INVALIDO`) atribuir/trocar plano para aluno não `ATIVO`; `PlanoDoAlunoSection` esconde o formulário e o botão de encerrar atribuição pelo mesmo motivo, sem esconder a leitura do histórico (preservada pela FIT-107). "Notas privadas do personal" não existe como feature nesta base — nada para bloquear. `createAssessment`/`createStudentCharge` deliberadamente não verificam `status` (avaliação/cobrança são domínios distintos, fora do que o pacote pede aqui). Detalhe completo na atualização de `ADR-009-ENCERRAMENTO-DE-VINCULO.md`. PR mergeado na main (22/09/2026).

`FIT-109` permanece a única História restante desta sprint — depende de resolver a restrição física `Student.userId @unique` (global), já sinalizada pela `ADR-009` desde a FIT-106.
