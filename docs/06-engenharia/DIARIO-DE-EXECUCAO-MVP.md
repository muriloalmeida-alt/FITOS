...
Issue [FIT-090] criada (#94, sub-issue de EPIC-12 #87). Implementado: `CommercialPlan`/`PlanBillingCycle` (schema + migration aditiva), versionamento por publicação (`publishPlanVersion` nunca edita uma versão existente — desativa a anterior e cria a próxima), índice único parcial (`WHERE active = true`) garantindo no máximo uma versão vendável por `(code, billingCycle)`. Seed sintético publica a hipótese comercial do pacote (Essencial/Profissional), sinalizada como hipótese, não preço definitivo. Docs: `PLANOS-COMERCIAIS.md` (novo), `ASSINATURA-SAAS.md` e README do módulo atualizados. Nenhuma UI nem integração com Asaas nesta história — chegam na FIT-091/092. PR aberto (#95), pendente de revisão/merge.

## EPIC-12 pausado na FIT-091 — bloqueio de rede real ao Asaas

Ao tentar iniciar a FIT-091 (#96, prova técnica do Asaas), a sessão confirmou (via `curl`/proxy status, não só via `WebFetch`) que `api.asaas.com` e `api-sandbox.asaas.com` são recusados com 403 de negação de política no próprio `CONNECT` — diferente da API Ninjas, onde só a documentação estava bloqueada, aqui nenhum host do Asaas é alcançável, com ou sem credencial. Apresentado a Murilo antes de qualquer código de integração (não fabriquei nenhum client Asaas baseado em memória não verificada, dado o risco de código de pagamento incorreto). Decisão: pausar o EPIC-12 aqui e seguir para o EPIC-13 (FitOS Livre, #97), cujas histórias FIT-100 a FIT-104 não dependem de pagamento — só FIT-105 depende do motor da Fase 2.1.

Registrado: Issue FIT-091 (#96, sub-issue de EPIC-12) marcada como bloqueada; ADR-003 atualizado com o achado; EPIC-12 doc atualizado com o estado pausado; Issue EPIC-13 (#97) criada.
