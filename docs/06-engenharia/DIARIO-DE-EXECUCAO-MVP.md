...
PR #117 aberto com o status `PRONTO PARA REVISÃO GPT/CODEX — NÃO MERGEAR`, Gate verde (841/841 testes, typecheck/lint/build limpos) e sem conflito de merge. Murilo pediu o merge; mergeado (commit `dbb8f93`) na `main`. O classificador de permissões da sessão sinalizou, ao tentar cancelar a inscrição de eventos do PR já mergeado, que o checkbox de revisão do próprio PR seguia desmarcado no momento do merge — verificado diretamente com Murilo (`AskUserQuestion`) se o merge estava dentro do que ele autorizou; ele confirmou que sim, sem necessidade de reverter.

**Importação real em homologação — confirmada.** Esta sandbox não tinha rede para `fitos-web-hml-homologacao.up.railway.app` (proxy de saída bloqueou) nem o valor do segredo, então a chamada final teve que ser feita pelo próprio Murilo: colar `https://fitos-web-hml-homologacao.up.railway.app/api/admin/catalog-import?source=curated&secret=SEU_SEGREDO` (com o valor real de `CATALOG_IMPORT_TRIGGER_SECRET`, obtido no painel Railway) direto na barra de endereço do navegador — um `GET` é exatamente isso. Murilo confirmou sucesso. Pendência registrada, não executada nesta sessão (fora do alcance de rede/acesso): rotacionar `CATALOG_IMPORT_TRIGGER_SECRET` no Railway, pela mesma regra já registrada para qualquer segredo usado por `?secret=` na URL (fica exposto em log de acesso/histórico do navegador a partir do primeiro uso).

## Bug-fix: seletor de exercício na montagem de treino (Issue #118)

Murilo reportou, já usando o catálogo com 208+ exercícios: "Ao montar o treino está muito difícil de selecionar o exercício."

**Investigação encontrou um bug real além da UX do `<select>` nativo.** `ItensDoModelo.tsx`/`ItensDoMeuTreino.tsx` chamavam `listCatalogExercises({ pageSize: 100 })` para abastecer o seletor — só que `MAX_CATALOG_PAGE_SIZE` (`exercises.ts`) satura exatamente em 100. Com 208 exercícios curados só na primeira fonte global (IMP-EX-002), mais da metade do catálogo já estava impossível de selecionar ao montar um treino, qualquer que fosse o widget — trocar por um autocomplete sem corrigir a busca de dados só teria trocado a superfície do mesmo bug.

**Decisão: `listCatalogExercisesForPicker` busca o catálogo completo, sem paginação — nunca reaproveita `listCatalogExercises`.** As duas funções têm contratos deliberadamente diferentes: a paginada serve a tela de navegação do catálogo (`/painel/exercicios`, onde paginar é o comportamento certo); a nova serve um seletor client-side que precisa do universo completo para filtrar localmente, sem round-trip de rede por tecla digitada. Um `PICKER_HARD_LIMIT` (1000) é só rede de segurança, não um valor esperado na escala atual.

**`ExerciseAutocomplete` (novo, `src/shared/ui/`)** — combobox acessível: filtro por texto client-side (insensível a maiúsculas/acentos), navegação por teclado (setas/Enter/Esc), botão de limpar seleção, e uma regra de correção central: texto digitado que nunca foi confirmado por clique/Enter é descartado ao perder o foco — o campo nunca fica com um texto visível que não corresponde ao `exerciseId` de fato selecionado (achado ao escrever o primeiro teste dessa interação: sem essa regra, um personal poderia digitar uma busca, mudar de ideia, clicar em "Adicionar" sem querer e o formulário enviaria o exercício selecionado antes, silenciosamente). Aplicado sem alteração de contrato em `ItensDoModelo.tsx` (personal) e `ItensDoMeuTreino.tsx` (individual) — os dois fluxos de montagem de treino desta base, estruturalmente idênticos desde a FIT-030/FIT-102.

Implementado:

- `listCatalogExercisesForPicker` (`exercises.ts`).
- `src/shared/ui/ExerciseAutocomplete.tsx` + `.module.css`, exportado do `index.ts`.
- `ItensDoModelo.tsx`/`ItensDoMeuTreino.tsx`: `<select>` nativo substituído pelo autocomplete; CSS morto (`.selectField`/`.selectLabel`/`.select`) removido dos dois módulos.
- `treinos/[id]/page.tsx`/`meus-treinos/[id]/page.tsx`: usam a nova função, sem o cap de 100.

Testado: 5 testes novos em `catalog.integration.test.ts` (mais de 100 itens retornados quando o catálogo do tenant tem mais de 100 — nunca limitado como `listCatalogExercises`; inclui músculo; exclui arquivado; exclui de outro tenant; inclui `FITOS_CURATED`), 8 testes do `ExerciseAutocomplete` (filtro, seleção por clique, navegação por teclado, botão de limpar, texto não confirmado descartado no blur, sem resultado, erro, desabilitado), 5+4 testes novos em `ItensDoModelo.test.tsx`/`ItensDoMeuTreino.test.tsx` (nenhum dos dois tinha teste próprio antes — só cobertura indireta via `page.test.tsx`) cobrindo o fluxo real de busca+seleção+POST. Suíte completa em 863/863, `tsc --noEmit`/`eslint .`/`npm run build` limpos. Fluxo real de ponta a ponta em navegador (Playwright contra o dev server): cadastro → criar modelo de treino → buscar "agachamento" no autocomplete (8 resultados visíveis, filtro instantâneo) → selecionar → adicionar → item aparece na lista com músculo e prescrição corretos, campo de busca volta a ficar vazio. Dados sintéticos (usuários/tenants/treinos de teste) removidos do banco depois da verificação.

PR #119 aberto com o status `PRONTO PARA REVISÃO GPT/CODEX — NÃO MERGEAR`, Gate verde (863/863 testes, typecheck/lint/build limpos) e sem conflito de merge. Murilo pediu o merge; mergeado (commit `dcab598`) na `main`. Issue #118 fechada.

## EPIC-14 — Landing comercial, onboarding completo e biblioteca ilustrada

Murilo enviou um pacote novo, em três arquivos ZIP por causa do tamanho (`FitOS_Pacote_Claude_Parte_1/2/3_de_3`): uma instrução única de implementação (`PROMPT_CLAUDE_LANDING_ONBOARDING_FITOS.md`), não um backlog numerado como os pacotes anteriores desta sessão. Pedi as três partes antes de começar — a primeira, sozinha, já avisava explicitamente "não execute a implementação antes de extrair todas as partes".

**Levantamento antes de qualquer código** (seção 12 do próprio pacote exige): despachei um agente de investigação para mapear a landing atual (`src/app/page.tsx` — só o placeholder da FIT-006, sem nenhum guard de auth a remover), o mecanismo de autenticação, o cadastro público (`?modo=individual`, FIT-101), o convite/ativação de aluno (FIT-015, já resolve praticamente todo o contrato que a seção 8 do pacote pede), o Design System M3 (paleta "chrome" navy/lima já em uso pelo `AppShell`, achado importante — é exatamente a direção visual escura que o pacote pede, sem precisar inventar nada), o catálogo de exercícios (sem campo de imagem hoje) e o catálogo de produtos do FitOS Livre (não existe em código — EPIC-12 nunca chegou a implementar isso).

Os 56 assets do pacote (19+19+18) batem exatamente com o manifesto (`MANIFESTO_ASSETS.csv`) — a única diferença é que os 19 da Parte 2 vieram em `.webp` "otimizada" (substituindo uma versão anterior em `.png`, conforme o próprio README daquela parte). Separei por conteúdo: 45 são ilustrações de exercícios de fato, 11 são fotos de marketing/mockups de painel (útil para a landing).

**Formalizado como EPIC-14** (Issue #120), não uma sub-história de EPIC-12/13 — pacote e origem diferentes, mesmo dependendo do que a FIT-015/FIT-101 já entregam. Sete Histórias (FIT-110 a FIT-116). **Adaptação de processo registrada**: o pacote pede "uma única PR"; a governança desta base já exige branch+PR por História com gate `NÃO MERGEAR` — preservei o processo já em uso (autorizado pela própria seção 12 do pacote: "preserve a arquitetura... registre a adaptação").

### FIT-110 — Landing page comercial

Issue #121, sub-issue de #120. Substitui `src/app/page.tsx` (placeholder) pela landing real: cabeçalho (`LandingHeader`, único Client Component da página — menu mobile), hero com o texto exato do pacote, três pilares de benefícios, os três caminhos do produto (seção 8A: Sou Personal / Treino com Personal / FitOS Livre), seção Personal-aluno, CTA final, rodapé.

**Decisão: toda a página herda a paleta "chrome" (navy/lima) do `AppShell` autenticado, redefinindo localmente os tokens semânticos que `Card`/`Button`/`TextField` já consomem** — mesmo padrão de sobreposição local que `AppShell.module.css` já usa para o botão "Sair" sobre o chrome fixo. Zero Design System novo, e o fundo fica escuro mesmo em preferência de sistema clara (os tokens chrome nunca alternam com o tema) — exatamente o efeito visual que a seção 4.2 do pacote pede.

**Duas fotos reais do pacote**, escolhidas por bater exatamente com os dois requisitos da seção 4.3 (verifiquei o conteúdo de cada imagem, não só o nome do arquivo): "Treino premium com personal trainer.png" (personal acompanhando aluno num lunge com halteres) como hero principal, "Treinador e aluno revisam o treino no celular.png" como secundária. Convertidas de PNG (~1,8 MB) para WebP (~40-65 KB, redimensionadas a 900px) — `public/media/landing/`, mesmo padrão de `public/media/brand/`.

**Terceiro caminho ("Treino com Personal") ganhou um campo real, não só texto**: a seção 8 do pacote pede, para quem não tem convite em mãos, "oferecer campo para inserir o código" — `ConviteCodeForm` (novo) só navega para `/ativar-conta?token=...`, reaproveitando a validação já existente da FIT-015 sem nenhum contrato novo. Encontrei e corrigi um efeito colateral real durante a verificação em navegador: o nome/rótulo do campo ("código de convite") aciona a heurística de autofill de código do Chrome, que injeta `caret-color` no input depois da hidratação — React reportava isso como divergência servidor/cliente a cada carregamento. `autoComplete="off"` resolveu (confirmado sem nenhum aviso depois).

**Rodapé sem link morto**: não existe página de Termos/Privacidade no repositório — texto de pendência ("em preparação"), nunca uma rota falsa, conforme a seção 5.5 do pacote exige explicitamente.

**Escopo deliberadamente sem o teaser da biblioteca ilustrada** (seção 8B do pacote também pede isso na landing) — adiado para a FIT-111, que estabelece o caminho de armazenamento real das imagens de exercício; construir um caminho temporário agora só para essa vitrine seria descartado depois.

Implementado: `src/app/page.tsx`/`page.module.css` (reescrita completa), `LandingHeader.tsx`/`.module.css` (novo), `ConviteCodeForm.tsx`/`.module.css` (novo), duas imagens otimizadas em `public/media/landing/`.

Testado: `page.test.tsx` (7), `LandingHeader.test.tsx` (3), `ConviteCodeForm.test.tsx` (3) — cabeçalho, hero com o texto exato do pacote, três pilares, três caminhos com links reais, seção Personal-aluno, nenhum link morto, alt text funcional em toda imagem, menu mobile abrindo/fechando/navegando, validação e navegação do código de convite. Suíte completa em 876/876, `tsc --noEmit`/`eslint .`/`npm run build` limpos. Fluxo real de ponta a ponta em navegador (Playwright contra o dev server, desktop 1400×1000 e mobile 390×844): hero, benefícios, três caminhos (incluindo o campo de convite), seção Personal-aluno, CTA final, rodapé, menu mobile abrindo e fechando — capturas em `docs/06-engenharia/evidencias/FIT-110/`. Achado registrado, fora do escopo (pré-existente): `/favicon.ico` retorna 404 em toda a aplicação, não só nesta página.
