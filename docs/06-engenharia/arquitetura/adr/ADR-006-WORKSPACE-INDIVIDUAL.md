...
## Atualização — a decisão adiada acima não foi necessária na FIT-102 (21/09/2026)

A FIT-102 ("Criar treino individual") chegou e a resposta foi mais simples do que o adiamento acima previa: **nenhuma decisão sobre `Student` foi necessária**, porque o escopo real da FIT-102 (pacote: "adicionar, remover, ordenar e parametrizar exercícios" — o *builder*) nunca toca `Student`, `PlanAssignment` ou `WorkoutSession`. `TrainingPlan`/`Workout`/`WorkoutExercise` só têm `tenantId` — `createWorkout`, `addWorkoutExercise`, `updateWorkoutExercise`, `removeWorkoutExercise`, `reorderWorkoutExercises`, `duplicateWorkout`, `listCatalogExercises` (FIT-030/FIT-023) já eram inteiramente genéricas por tenant, sem nenhuma lógica específica de `PERSONAL` — foram reaproveitadas **sem alterar uma linha** desses módulos, só com rotas/páginas próprias (`/api/meus-treinos`, `/painel/meus-treinos`) gateadas por `requireIndividual()` em vez de `requirePersonal()`. O "bucket rascunho" (`ensureDraftTrainingPlanForTenant`, já existente desde a FIT-030) resolveu de graça a exigência de "o treino pertence ao praticante, sem noção de plano visível" — é exatamente esse o comportamento que ele já tinha para o personal antes de a FIT-032 existir.

A decisão real adiada por este ADR — `Student` como auto-referência vs. outra modelagem — só volta a ser necessária na **FIT-103** ("Executar treino individual"), porque `WorkoutSession.studentId` é uma FK obrigatória: é ali, não na FIT-102, que a pergunta "quem é o `Student` de um praticante sem aluno" precisa de resposta.

## Atualização — a decisão de `Student` finalmente resolvida na FIT-103 (22/09/2026)

A FIT-103 ("Executar treino individual") chegou e a decisão adiada desde o topo deste ADR foi tomada: **`Student` como auto-referência** (`Student.userId === Tenant.ownerId`), criado sob demanda por `ensureStudentForIndividual` (`src/modules/tenancy/ensureStudentForIndividual.ts`, mesmo padrão de idempotência/concorrência de `ensureTenantForPersonal`/`ensureTenantForIndividual`) — nunca no cadastro ou no onboarding, só quando o praticante de fato começa um treino pela primeira vez. Nenhuma UI de "perfil de aluno" é exposta a ele; o `Student` é puramente um detalhe de implementação que satisfaz a FK obrigatória de `WorkoutSession`.

**A alternativa rejeitada e por quê**: reaproveitar `startOrResumeWorkoutSession` (FIT-041) sem alteração exigiria que o treino do praticante passasse pelo mesmo pipeline de `PlanAssignment` + snapshot imutável da ADR-005 — ou seja, o praticante teria que "se atribuir" o próprio treino antes de poder executá-lo, uma ceremônia inteira desenhada para proteger o aluno de o personal alterar a prescrição depois de atribuída. Esse risco simplesmente não existe quando quem prescreve e quem executa são a mesma pessoa. Em vez de forçar essa ceremônia, `startOrResumeIndividualWorkoutSession` (nova função em `sessions.ts`, ao lado de `startOrResumeWorkoutSession`) troca só a validação de acesso — exige que `workoutId` pertença ao próprio tenant e esteja `ATIVO` (mesmo padrão de posse de `getWorkoutForTenant`, já usado pelo builder da FIT-102) — e reaproveita **sem nenhuma alteração** todo o resto do motor de execução: `recordSessionResult`, `completeWorkoutSession`, `abandonWorkoutSession`, `getInProgressSessionForStudent` já eram genéricas por `studentId`, sem nenhuma lógica de `PERSONAL`/`ALUNO` embutida.

Rotas próprias (`/api/minhas-sessoes/*`) e páginas próprias (`/painel/meus-treinos/sessao`, `/painel/meus-treinos/[id]` com o botão "Começar treino") — mesma arquitetura "própria e inequívoca" já seguida pela FIT-101/FIT-102, nunca reaproveitando as rotas/páginas do aluno (`/api/workout-sessions`, `/painel/treino/sessao`) diretamente.
