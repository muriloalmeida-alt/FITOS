...
`buildDeepClone` (usado tanto pela duplicação de modelo da FIT-031 quanto pela atribuição da FIT-033) clona `TrainingPlan`/`Workout`/`WorkoutExercise` gerando ids novos via `cuid()` — nunca reaproveita ids do original, nunca cria vínculo de FK entre a cópia e a origem (não há necessidade de rastrear proveniência para o MVP; se um dia for necessário auditar "de qual modelo esta atribuição veio", isso ficaria em `AuditEvent`, não em uma FK estrutural).
