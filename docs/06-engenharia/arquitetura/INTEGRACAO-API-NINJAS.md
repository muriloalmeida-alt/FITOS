...
**Risco adicional aceito do segredo por `?secret=` na query string**: diferente do header, um valor na URL **fica gravado em log de acesso** (aplicação, proxy, Railway — geralmente por semanas), pode aparecer no histórico do navegador de quem abrir a URL, e pode ser exposto via `Referer` se essa página carregar algo externo. Murilo foi informado desse risco — maior que o do `GET` isolado — e decidiu aceitá-lo em 21/09/2026 porque a ferramenta disponível (navegador/app/webhook) não permitia adicionar um header customizado. **Consequência obrigatória**: qualquer valor de `CATALOG_IMPORT_TRIGGER_SECRET` usado por URL deve ser tratado como permanentemente exposto a partir do primeiro uso — rotacionar (gerar um valor novo e reconfigurar no Railway) assim que a carga real terminar, nunca reutilizar esse mesmo valor depois.
