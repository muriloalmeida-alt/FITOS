...
- `CATALOG_IMPORT_TRIGGER_SECRET` (exceção de governança do IMP-EX-001, ver `INTEGRACAO-API-NINJAS.md`): só deve existir enquanto a rota `POST /api/admin/catalog-import` for necessária (acesso SSH/CLI ao Railway indisponível em 21/09/2026) — remover a variável assim que a carga real for concluída e confirmada, ou assim que o acesso SSH/CLI for restabelecido.
