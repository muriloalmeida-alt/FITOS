...
export function ItensDoModelo({ workoutId, items, catalog }: ItensDoModeloProps) {
  ...
  async function handleAddSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/workouts/${workoutId}/itens`, {
      method: "POST",
  ...
  async function handleRemove(itemId: string) {
    ...
    const response = await fetch(`/api/workouts/${workoutId}/itens/${itemId}`, { method: "DELETE" });
  ...
  async function handleMove(itemId: string, direction: "up" | "down") {
    ...
    setRowError(null);
    setPendingItemId(itemId);
    const response = await fetch(`/api/workouts/${workoutId}/itens/reordenar`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className={styles.container}>
      {items.length === 0 ? (
        ...
        <ul className={styles.list} aria-label="Exercícios do modelo, em ordem">
