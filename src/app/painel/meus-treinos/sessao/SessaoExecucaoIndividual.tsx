...
import styles from "./SessaoExecucaoIndividual.module.css";
...
interface SessaoExecucaoIndividualProps {
  sessionId: string;
...
/// Mesmo temporizador de descanso de `SessaoExecucao` (FIT-041).
...
/// Execução da sessão individual (FIT-103) — mesmo motor de `SessaoExecucao`
/// (FIT-041), só troca a base da API (`/api/minhas-sessoes` em vez de
/// `/api/workout-sessions`).
export function SessaoExecucaoIndividual({ sessionId, items }: SessaoExecucaoIndividualProps) {
  ...
  async function salvarResultado(item: SessionItemProp) {
    ...
    try {
      const response = await fetch(`/api/minhas-sessoes/${sessionId}/resultados`, {
        method: "POST",
  ...
  async function finalizar(action: "concluir" | "abandonar") {
    ...
    setIsFinishing(true);
    try {
      const response = await fetch(`/api/minhas-sessoes/${sessionId}/${action}`, { method: "POST" });
      if (!response.ok) {
        const body = await response.json().catch(() => null);
