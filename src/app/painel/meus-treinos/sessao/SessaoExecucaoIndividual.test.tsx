...
import { SessaoExecucaoIndividual, type SessionItemProp } from "./SessaoExecucaoIndividual";
...
describe("SessaoExecucaoIndividual (FIT-103)", () => {
  afterEach(() => {
    vi.resetAllMocks();
    vi.unstubAllGlobals();
  });
  ...
  it("renderiza o item com parâmetros, notas e instruções legíveis", () => {
    render(<SessaoExecucaoIndividual sessionId="sess1" items={[ITEM]} />);
  ...
  it("estado vazio honesto quando o treino não tem exercícios", () => {
    render(<SessaoExecucaoIndividual sessionId="sess1" items={[]} />);
  ...
  it("'Salvar resultado' envia o resultado à API individual e mostra o selo 'Salvo'", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/minhas-sessoes/sess1/resultados", {
      method: "POST",
  ...
  it("'Concluir treino' envia a conclusão à API individual e navega para /painel", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/minhas-sessoes/sess1/concluir", { method: "POST" });
    await waitFor(() => expect(push).toHaveBeenCalledWith("/painel"));
  });

  it("'Abandonar treino' envia o abandono à API individual e navega para /painel", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/minhas-sessoes/sess1/abandonar", { method: "POST" });
    await waitFor(() => expect(push).toHaveBeenCalledWith("/painel"));
  });
});
