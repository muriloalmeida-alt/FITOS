...
describe("POST /api/workouts/[id]/itens/reordenar", () => {
  afterEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("retorna 400 quando a lista não corresponde aos itens existentes", async () => {
    const { WorkoutError } = await vi.importActual<typeof import("@/modules/workouts/workouts")>("@/modules/workouts/workouts");
    requirePersonal.mockResolvedValue({ userId: "u1", role: "PERSONAL", tenantId: "tenant-real" });
    reorderWorkoutExercises.mockRejectedValue(new WorkoutError("VALIDACAO", "A nova ordem precisa conter exatamente os itens já existentes no modelo."));

    const { POST } = await import("./route");
    const response = await POST(
      new Request("http://localhost/x", { method: "POST", body: JSON.stringify({ orderedIds: ["a"] }) }),
      { params: makeParams("w1") }
    );
